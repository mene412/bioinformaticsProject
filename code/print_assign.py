...
def fastCLARANS(samples, k, numlocal, maxneighbor, metric='braycurtis'):
    ...
    def assign(centers, samples, metric='jaccard'):
        ...
        """
        with open("distance_timings.log", "a") as log_file:
            log_file.write(f"\nAssign called with centers: {centers}\n")
            
            for sample in range(len(samples)):
                dists = []
                for i, center_idx in enumerate(centers):
                    start_time = time.time()
                    dist = get_cached_distance(sample, center_idx, metric)
                    elapsed = time.time() - start_time

                    log_file.write(
                        f"Sample {sample} -> Center {center_idx}: {elapsed:.6f} sec\n"
                    )
                ...
                samples[sample]['label'] = dists[0][1]
...
def main():
    ...
    dataset_folder_path = "/nfsd/bcb/bcbg/berleselor/dataset"               # Replace with actual path
    ...
    numlocal = 2                           # Number of local minima to search
    maxneighbor = 5                       # Max neighbors per local search
